...
export function StatCard({ label, value, icon: Icon, tone = 'amber', hint, className }: StatCardProps) {
  ...
  return (
    <div
      className={cn(
        'group glass relative flex min-w-0 flex-col overflow-hidden rounded-2xl p-4 sm:p-5 transition-all duration-300 ease-out hover:border-primary/40 hover:shadow-md [container-type:inline-size]',
        className
      )}
    >
      {/* left accent rule */}
