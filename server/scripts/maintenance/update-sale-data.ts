...
async function matchAndMerge(data: any[], brokerName: string | null) {
  let brokerId = null;
  if (brokerName) {
    const b = await prisma.broker.findFirst({ where: { name: { contains: brokerName } } });
    if (b) brokerId = b.id;
  }
  ...
  for (const item of data) {
    ...
    if (!party) {
      console.log('Party not found:', item.company);
      continue;
    }

    // Attempt to find SaleOrder
    const order = await prisma.saleOrder.findFirst({
      where: {
        buyerId: party.id,
        saleDate: {
          gte: new Date(saleDate.getTime() - 86400000 * 2), // within 2 days
          lte: new Date(saleDate.getTime() + 86400000 * 2),
        }
      },
      include: {
        dispatches: true
    ...
    if (order) {
      // Calculate due days if both invoice and due dates exist
      let dueDays = null;
      ...
      // Update the dispatch with lorry and invoice
      if (order.dispatches.length > 0) {
        await prisma.saleDispatch.update({
          where: { id: order.dispatches[0].id },
          data: {
            vehicleNumber: item.lorry || order.dispatches[0].vehicleNumber,
            invoiceNumber: item.inv || order.dispatches[0].invoiceNumber,
            invoiceDate: invoiceDate,
            weightKg: item.rvpKata || order.dispatches[0].weightKg,
          }
        });
      }
      console.log('Updated order for', item.company, item.date);
      found++;
    } else {
      console.log('Order not found for', item.company, item.date, item.tonnage);
    }
  }
...
async function run() {
  console.log('Processing Mustafa...');
  await matchAndMerge(mustafa, 'Mustafa');
  console.log('Processing Jithu...');
  await matchAndMerge(jithu, 'Jithu');
  console.log('Processing RVP...');
  await matchAndMerge(rvp, null);
