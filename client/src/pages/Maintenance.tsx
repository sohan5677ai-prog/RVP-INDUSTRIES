...
export default function Maintenance({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) => api(`/maintenance-expenses/${id}`, { method: 'PUT', body }),
    onSuccess: () => {
  ...
  function openEdit(r: MaintenanceExpense) {
    setEditingId(r.id);
    ...
    <div className="space-y-7">
      ...
      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditingId(null); }}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle>{editingId ? 'Edit' : 'Record'} Maintenance Expense</DialogTitle></DialogHeader>
