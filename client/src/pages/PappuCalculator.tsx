...
import { Scale, Calculator, ArrowRightLeft, Percent } from 'lucide-react';
...
export default function PappuCalculator() {
  ...
  // Net cost of processing Pappu
  const netPappuCost = Math.max(0, totalGrossCost - totalCredits);

  // Effective cost of White Pappu (per kg and per tonne)
  const effectiveCostPerKg = pappuWeight > 0 ? netPappuCost / pappuWeight : 0;
  const effectiveCostPerTonne = effectiveCostPerKg * 1000;

  // Target Selling Price based on selected Profit Margin
  const targetPricePerKg = effectiveCostPerKg * (1 + marginPct / 100);
  const totalRevenue = targetPricePerKg * pappuWeight + totalCredits;
  const netProfit = totalRevenue - totalGrossCost;
            ...
            <Card className="bg-primary/[0.02] border border-primary/20 shadow-sm">
              <CardContent className="pt-5 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">Equals {rupees(effectiveCostPerTonne)} per Tonne</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card border shadow-sm">
              <CardContent className="pt-5 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Target Selling Price ({marginPct}%)</span>
                  <div className="text-3xl font-extrabold text-emerald-600 dark:text-emerald-500 tracking-tight mt-1">
                    {rupees(targetPricePerKg)} <span className="text-sm font-semibold text-muted-foreground">/ kg</span>
                  </div>
            ...
            <CardContent className="pt-4 space-y-4">
              ...
              <div className="space-y-2 text-sm pt-4 border-t-2 border-dashed">
                <div className="flex justify-between items-center text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">{rupees(netPappuCost)}</span>
                </div>
                <div className="flex justify-between items-center text-muted-foreground">
                ...
                <div className="flex justify-between items-center pt-3 mt-1 border-t text-base font-bold text-primary">
                  <span>Effective Pappu Value ({pappuWeight} kg @ {rupees(targetPricePerKg)}/kg)</span>
                  <span>{rupees(targetPricePerKg * pappuWeight)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
