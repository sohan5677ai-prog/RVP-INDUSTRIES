...
async function fixMissingSales() {
  ...
  const processGroup = async (arr, brokerName) => {
    ...
    for (const item of arr) {
      ...
      if (orderToUpdate) {
         ...
         console.log('RE-CREATING missing order for', item.company, invString, product);
  ...
  console.log('Valid order IDs processed:', validOrderIds.length);
  const finalOrders = await prisma.saleOrder.findMany();
  console.log('Final DB Count:', finalOrders.length);
}

fixMissingSales().catch(console.error).finally(() => prisma.$disconnect());
